	...
	<body>
		...
		<div id="show-auteur" class="content scaffold-show" role="main">
			...
			<ol class="property-list auteur">
				...
				<g:if test="${auteurInstance?.livresEdites}">
				<li class="fieldcontain">
					<span id="livresEdites-label" class="property-label"><g:message code="auteur.livresEdites.label" default="Livres Edites" /></span>
					
						<g:each in="${auteurInstance.livresEdites}" var="l">
						<span class="property-value" aria-labelledby="livresEdites-label"><g:link controller="livre" action="show" id="${l.id}">${l?.encodeAsHTML()}</g:link></span>
						</g:each>
